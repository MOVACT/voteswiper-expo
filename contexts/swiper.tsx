...
const SwiperProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  const [election, setElection] = React.useState<null | ElectionFull>(null);
  const [country, setCountry] = React.useState<Context["country"]>(null);
  ...
  const toggleDoubleWeight = React.useCallback(
    (id: number) => {
      ...
      answersCopy[election!.id][id].doubleWeight =
        !answersCopy[election!.id][id].doubleWeight;
